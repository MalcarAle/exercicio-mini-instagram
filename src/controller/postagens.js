...
const curtir = async (req, res) => {
  ...
  try {
    ...
    if (jaCurtiu) {
      return res.status(404).json("Usuario já curtiu esta postagem!")
    }
    ...
    if (!curtida) {
      return res.status(400).json("Falha ao curtir a postagem!")
    }
...
const comentar = async (req, res) => {
  ...
  if (!texto) {
    return res.status(404).json("Não é possível comentário vazio")
  }
  try {
    const postagem = await knex("postagens").where({ id: postagemId }).first()
    ...
    if (!comentario) {
      return res.status(400).json("Falha ao comentar a postagem!")
    }
...
const feed = async (req, res) => {
  ...
  try {
    // const postagens = knex("postagens").limit(10).offset(o)
